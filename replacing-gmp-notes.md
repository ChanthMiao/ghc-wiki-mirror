...
- some C--/Cmm (this is fairly basic if you know C well, though the same adage for knowing C well holds for C--: if you know Assembler well enough to  understand and debug C in it you will be much better off), the only real documentation on C-- itself is in the [C-- manual (PDF)](https://www.cs.tufts.edu/~nr/c--/extern/man2.pdf), from www.cs.tufts.edu/~nr/c--/; the implementation of C-- for GHC is performed by several Haskell modules in the directory [compiler/cmm/](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/cmm/) of the HEAD branch, see [ http://darcs.haskell.org/ghc](http://darcs.haskell.org/ghc)), and see [the new Commentary Cmm page](commentary/compiler/cmm-type); and,
...
>
>
> The current GMP implementation of Integer, defined in [libraries/base/GHC/Num.lhs](https://gitlab.haskell.org/ghc/ghc/blob/master/libraries/base/GHC/Num.lhs), is:
>
>
> ```
> data Integer
...
- [includes/Cmm.h](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/Cmm.h) (*Modify*: cpp test for `#if SIZEOF_mp_limb_t != SIZEOF_VOID_P `)
- [includes/MachRegs.h](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/MachRegs.h) (*Reference*: general; unrelated to GMP: may be starting point for vectorized Cmm (currently only -fvia-c allows auto-vectorization))
- [includes/mkDerivedConstants.c](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/mkDerivedConstants.c) (*Modify*: references to GMP `__mpz_struct`: `struct_size(MP_INT)`, `struct_field(MP_INT,_mp_alloc)`, `struct_field(MP_INT,_mp_size)`, `struct_field(MP_INT,_mp_d)` and `ctype(mp_limb_t)`.  Note: mp_limb_t generally == unsigned long)
- [includes/Regs.h](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/Regs.h) (*Modify*: references to MP_INT, `#include "gmp.h"`; Reference: Stg registers, etc.)
- [includes/Rts.h](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/Rts.h) (*Modify*: reference to `#include "gmp.h"`, `extern` declarations to `__decodeDouble` and `__decodeFloat`; References to various Stg types and macros)
- [includes/StgMiscClosures.h](https://gitlab.haskell.org/ghc/ghc/blob/master/includes/StgMiscClosures.h) (*Modify*: references to `RTS_FUN(...Integer)` PrimOps; *Reference*: Weak Pointers, other Stg closures)

- [rts/Linker.c](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/Linker.c) (*Modify*: `SymX(__gmpn...)` and related GMP functions)
- [rts/Makefile](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/Makefile) (*Modify*: building GMP library)
- [rts/PrimOps.cmm](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/PrimOps.cmm) (*Modify*: remove GMP references; NOTE: optimisation of `/* ToDo: this is shockingly inefficient */`, see discussion below)
- [rts/StgPrimFloat.c](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/StgPrimFloat.c) (*Modify*: `__encodeDouble`, `__encodeFloat` and `decode` versions defined here refer to GMP; might optimise with bitwise conversion instead of union; conversion depends on whether replacement MP library uses floating point, etc.)
- [rts/sm/Storage.c](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/sm/Storage.c) (*Modify*: `stgAllocForGMP`, `stgReallocForGMP` and `stgDeallocForGMP`; `mp_set_memory_functions(...)`; functions on lines 811, 833, 835, 848; may use as reference for implementation if replacement MP library uses GHC-garbage collected memory)
- [rts/gmp/](https://gitlab.haskell.org/ghc/ghc/blob/master/rts/gmp/) (directory) (*Modify*: recommended to remove entirely, i.e., do not add conditional compilation for users who want to keep on using GMP)
...
(2) Primitive Operations in [compiler/codeGen/CgPrimOp.hs](https://gitlab.haskell.org/ghc/ghc/blob/master/compiler/codeGen/CgPrimOp.hs)
