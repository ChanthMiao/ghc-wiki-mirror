...
Again you can call `someMeth` and yield a `String`: merely by supplying a `String` as second argument -- despite the FunDep+first instance giving that should be `Bool`.[improvement vs instance selection q's](Functional-dependencies-in-GHC/Key-examples/improvement-vs-instance-selection)
