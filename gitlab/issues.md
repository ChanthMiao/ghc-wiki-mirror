 ...
 * Is the **milestone appropriate**? If the patch is low-risk or the issue severe, perhaps it should be backported to the stable branch. In this case set the milestone to the next stable release and apply the ~"backport needed" label to the appropriate merge requests. If a backport is needed **do not close** the issue until the label has been applied.
